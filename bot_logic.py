...
class TradingBot:
    def __init__(self, symbol, trade_amount, short_window=SHORT_WINDOW, long_window=LONG_WINDOW):
        ...
        # Load historical data to initialize price history
        self.load_historical_data()

    def load_historical_data(self):
        """
        Fetches historical price data from yfinance to initialize price history.
        """
        ...
        try:
            ...
            if data.empty:
                logging.warning("No valid historical data retrieved. Starting with an empty price history.")
                return
    ...
    def run_strategy(self):
        ...
        # logging.debug(f"Updated price history: {self.price_history}")
        if len(self.price_history) > self.long_window:
        ...
        logging.info(f"Short MA: {short_ma:.2f}, Long MA: {long_ma:.2f}")

        # --- Strategy Logic ---
        if not self.has_position and short_ma > long_ma:
            # Buy signal: Short MA crosses above Long MA
            logging.info(f"Buy signal: Short MA ({short_ma:.2f}) > Long MA ({long_ma:.2f}).")
            if place_order(self.symbol, 'buy', self.trade_amount):
                self.has_position = True

        elif self.has_position and short_ma < long_ma:
            # Sell signal: Short MA crosses below Long MA
            logging.info(f"Sell signal: Short MA ({short_ma:.2f}) < Long MA ({long_ma:.2f}).")
